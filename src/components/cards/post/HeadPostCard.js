...
const HeadPostCard = ({ post }) => {
  ...
  return (
  <div className="card shadow-none" style={{width: "300px", right: "0px", top:"2px", position: "absolute"}}>
...
<Link to={`/post/${slug}`} >
  ...
  position: "absolute", bottom:"120px", right:"50%"}} > 
  {title}</p>
</Link>
            
    <p className="card-text">{renderHTML(`${(description.substring(0, 70))}...`)}</p>
  </div>
